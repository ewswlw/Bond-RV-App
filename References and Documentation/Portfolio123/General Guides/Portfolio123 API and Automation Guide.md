...
- Reference this guide when automating Portfolio123 workflows via the REST API or `p123api` Python package.
- Use it before generating new credentials or granting API access to ensure security, rate limits, and credit costs are understood.
- Apply it during integration projects for endpoint coverage, code examples, and troubleshooting strategies.
- Consult it when documenting automation standards so every agent follows consistent authentication and error-handling patterns.
- If you operate only through the UI, you may not need this; once programmatic control is required, treat it as essential.
