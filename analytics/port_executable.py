...
Z Score. After selecting top N pairs, filters to only pairs where cusip_1 matches
CUSIPs from runs_today.csv with CR01 @ Wide Offer >= 2000, and cusip_2 is in the
portfolio CUSIP list for monitoring relative value opportunities.
"""
...
def run_analysis(
    ...
    # Filter to only pairs where cusip_1 is in executable CUSIPs (CR01 @ Wide Offer >= threshold)
    executable_cusips_in_data = executable_cusips & set(cusips)
    print(
        f"\nFiltering to pairs where cusip_1 has CR01 @ Wide Offer >= {CR01_WIDE_OFFER_THRESHOLD} "
        f"({len(executable_cusips_in_data)} executable CUSIPs)..."
    )
    before_executable_filter_count = len(results_df)
    results_df = results_df[results_df["cusip_1"].isin(executable_cusips_in_data)].copy()
    after_executable_filter_count = len(results_df)
    print(f"Filtered from {before_executable_filter_count:,} to {after_executable_filter_count:,} pairs")
