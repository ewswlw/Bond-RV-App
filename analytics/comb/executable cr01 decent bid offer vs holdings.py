...
- Universe CUSIPs from bql.parquet (filtered by runs_today.csv: CR01 @ Wide Offer > 2000, Bid/Offer>3mm < 3)
- Holdings CUSIPs from runs_today.csv (CR01 @ Tight Bid > 2000, Bid/Offer>3mm < 3) matched with portfolio (last date)
...
def load_cr01_holdings(
    ...
    required_cols = ["CUSIP", "CR01 @ Tight Bid", "Bid/Offer>3mm"]
...
def load_cr01_universe(
    ...
    # Check required columns exist
    required_cols = ["CUSIP", "CR01 @ Wide Offer", "Bid/Offer>3mm"]
    missing_cols = [col for col in required_cols if col not in runs_df.columns]
    if missing_cols:
        raise ValueError(f"Missing required columns in runs_today.csv: {missing_cols}")
    ...
    # Filter: CR01 @ Wide Offer > 2000 and Bid/Offer>3mm < 3
    # Exclude rows with NaN/null values
    filtered_runs = runs_df[
    ...
    if filtered_runs.empty:
        raise ValueError(
            "No CUSIPs found in runs_today.csv matching criteria: "
            "CR01 @ Wide Offer > 2000 and Bid/Offer>3mm < 3"
        )
...
def run_analysis(
    ...
    output_path = output_dir / "executable cr01 decent bid offer vs holdings.csv"
